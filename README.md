...
以下是从用户视角整理的候选功能，尚未实现，按优先级大致排序。欢迎按需取用或调整顺序。

- [ ] **画质快捷筛选 + 关键词高亮** — 4K / 1080p / 720p 快捷标签，比"名称含"手输更顺手；标题里命中搜索词做高亮。（纯前端，改动小，影视场景高频）
- [ ] **搜索历史 / 收藏** — 最近搜索词下拉；收藏夹保存感兴趣的种子（magnet 存 localStorage）。（纯前端，天天用）
- [ ] **引擎分组 / 预设** — 40 个引擎按"动漫 / 影视 / 综合 / 成人"分组，一键切换；选中的引擎与筛选条件存 localStorage，下次打开还在。
- [ ] **批量操作** — 勾选多条 → 一键批量推送 qBittorrent、批量复制磁力、导出 CSV。（找整季剧集很实用）
- [ ] **详情预览** — 点开看文件列表、大小分布（复用已有的 `detailUrl` / `resolveMagnet` 扩展），避免下到"挂羊头卖狗肉"的种子。
